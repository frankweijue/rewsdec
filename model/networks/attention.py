...
class AttentionType2(Attention):
    ...
    def _forward_pe(self, feature, hidden, mask):
        ...
        pe_index = (torch.arange(0, feature.size(1)).unsqueeze(0).repeat(feature.size(0),
                                                                         1)).cuda()  # (batch, seq_len), float
...
class AttentionMask(nn.Module):
    ...
    def _forwardc(self, feature, hidden, segment, mask):
        ...
        mask_index = FloatTensor(range(seq_len)).expand(batch_size, seq_len).unsqueeze(2)  # batch_size, seq_len, 1
