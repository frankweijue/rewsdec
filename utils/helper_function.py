    ...
    for idx1, idx2 in enumerate(ts_gather_idx.cpu().data.numpy().tolist()):
        ...
        if idx2 not in ts_refined:
            ...
            if max_iou > iou_thres:
                ...
                ts_new_gather_idx.append(idx2)
    #print(torch.stack(list(chain(*ts_refined.values())), dim=0))

    try:
        ts_refined=se2cw(torch.stack(list(chain(*ts_refined.values())), dim=0))
    except:
        ts_refined=torch.Tensor([])

    return ts_refined, Variable(LongTensor(ts_new_gather_idx))
